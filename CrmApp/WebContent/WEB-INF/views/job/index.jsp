...
<div class="container-fluid">
	<div class="row bg-title">
		...
		<div class="col-lg-9 col-sm-8 col-md-8 col-xs-12 text-right">
			<a href="<c:url value="${ UrlConstants.URL_JOB_ADD }" />" 
					...
					<table class="table" id="example">
						...
						<c:forEach items="${ jobs }" var="item" varStatus="loop">
							<tr>
								...
								<td>
									<a 
										href="<c:url value="${ UrlConstants.URL_JOB_EDIT }?id=${ item.id }" />" 
										class="btn btn-sm btn-primary">Sửa
									</a> 
									<a 
										href="<c:url value="${ UrlConstants.URL_JOB_DELETE }?id=${ item.id }" />" 
										class="btn btn-sm btn-danger">Xóa
									</a>
									<a 
										href="<c:url value="${ UrlConstants.URL_JOB_DETAILS }?id=${ item.id }" />" 
